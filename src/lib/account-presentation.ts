...
export function accountPresentation(products: readonly Product[]) {
  ...
  const orders = [
    { id: 'EX-1024', status: 'pending' as const, date: '14.09.2026', items: items.slice(2), totalMinor: total(items.slice(2)) },
    { id: 'EX-1023', status: 'delivered' as const, date: '13.09.2026', items: items.slice(0, 2), totalMinor: total(items.slice(0, 2)) },
  ];
  ...
  return {
    ...
    operations: [
      ...
      { id: 'EX-1022', label: 'Пополнение', detail: 'Банковская карта', date: '13.09.2026', amountMinor: creditMinor },
    ],
