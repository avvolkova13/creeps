...
const messages = {
  unavailable: { title: "Товары временно недоступны", text: "Сейчас не получается показать товары и их наличие. Загляните позже." },
  ...
  return (
    <div className="catalog-status" role="status" aria-busy={status === "loading"}>
      ...
      <div><h3>{product && status === "unavailable" ? "Не удалось загрузить товар" : message.title}</h3><p>{message.text}</p>
        {product && <Link className="text-link" href="/catalog">Вернуться в каталог <UiIcon name="arrow" /></Link>}
      </div>
    </div>
  );
}
